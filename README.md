...
flowchart TD
    subgraph UI ["User Interface (Streamlit)"]
        CB[AI Legal Chatbot]
        RAG_UI[Document RAG Q&A]
        CG[Contract Generator]
        NG[Notice & Affidavit Generator]
        RA[Risk Auditor]
        CC[Contract Comparison]
    end

    subgraph RAG_Pipeline ["RAG & Embedding Pipeline"]
        RAG_UI -->|Upload PDF| Chunk[Text Chunks + Page Metadata]
        Chunk --> Emb[HuggingFace Embeddings]
        Emb --> VDB[(ChromaDB VectorStore)]
    end

    subgraph LLM_Engine ["Model Router"]
        CB --> Router{Model Router}
        VDB --> Router
        CG --> Router
        NG --> Router
        RA --> Router
        ...
        Router -->|Cloud| Groq[Groq Cloud API]
    end

    subgraph Output ["Exports & Answers"]
        Ollama --> Res[Cited Answers / Word & PDF Exports]
        Groq --> Res
    end
```
...
```bash
# 1. Clone repository
git clone https://github.com/your-username/RAG-Based-Legal-Document-Assistant.git
cd RAG-Based-Legal-Document-Assistant
